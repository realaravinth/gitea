...
func TestDumpRestore(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, u *url.URL) {
		...
		assert.NoError(t, exec.Command("sed", "-ie", "s/"+reponame+"/"+newreponame+"/g", filepath.Join(d, "repo.yml")).Run())
		output, err := exec.Command("diff", "-u", filepath.Join(d, "repo.yml"), filepath.Join(newd, "repo.yml")).CombinedOutput()
		if err != nil {
